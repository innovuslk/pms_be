...
router.post('/getAPILineData', async (req, res) => {
    try {
        ...
        const totalPieceCountQuery = "SELECT SUM(pieceCount) as totalPieceCount FROM api_piececount WHERE operator = 'end line checking' AND DATE(timestamp) = ?";
        const totalPieceCountValues = [current_date];
