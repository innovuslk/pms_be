...
router.post('/getPlannedTarget', async (req, res) => {
    try {
        ...
        const lineNoQuery = "SELECT plantName from operatorDailyAssignment WHERE userid = ? AND date = ?";
        ...
        if (!lineNoResult.length) {
            return res.status(404).send("PlantName not assigned for today");
        }
        const plantName = lineNoResult[0].plantName;



        // Retrieve sales orders, line items, and quantities for the current date
        const dailyPlanQuery = `
            SELECT plannedTarget
            FROM dailyPlan
            WHERE date = ? AND plantName = ?;
        `;
        const dailyPlanValues = [current_date, plantName];
        ...
        if (dailyPlanResult.length > 0) {
            const dailyTarget = dailyPlanResult[0].plannedTarget;
            // Respond with success and the total piece count
            res.status(200).json({ message: 'plannedTarget recieved successfully.', dailyTarget: dailyTarget });
        } else {
